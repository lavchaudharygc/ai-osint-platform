...
<body>
    ...
    ${(data.telegram_cti && data.telegram_cti.total_records > 0)
        ? `<p class="finding-critical">⚠ Telegram CTI Breach Intelligence matched ${data.telegram_cti.total_records} compromised records across ${data.telegram_cti.databases.length} databases.</p>`
        : '<p style="color:#555;">No critical threat indicators or leak database triggers identified.</p>'}
